...
case class Position(x:Int, y:Int){
  def shiftPosition(dir:Char) = if (dir == 'R') Position(x, y+1) else Position(x-1,y)
}
...
def scrabble(result: Int, point: Position, input: List[Char], dw: Int, tw: Int, dir:Char):Int ={
  input match {
    ...
    case x :: y if grid(point.x)(point.y).equals("DL") => scrabble(result + score(x - 97) * 2, point.shiftPosition(dir), y, dw, tw, dir)
    case x :: y if grid(point.x)(point.y).equals("TL") => scrabble(result + score(x - 97) * 3, point.shiftPosition(dir), y, dw, tw, dir)
    case x :: y => scrabble(result + score(x - 97), point.shiftPosition(dir), y, dw, tw, dir)
  }
}

scrabble(0, Position(8, 7), "bootcamp".toCharArray.toList, 1, 1, 'D')
scrabble(0, Position(14, 14), "cryptozoologies".toCharArray.toList, 1, 1, 'D')
scrabble(0, Position(0,0), "indix".toCharArray.toList, 1, 1, 'R')
scrabble(0, Position(0,0), "unconsciousness".toCharArray.toList, 1, 1, 'R')
